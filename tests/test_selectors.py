...
def test_every_n_row(df: DataFrameLike, row_selector, expected):
    res = GT(df).fmt_integer(columns="col1", rows=row_selector, scale_by=100)
...
def test_every_n_row_raise(df: DataFrameLike):
    with pytest.raises(ValueError) as exc_info:
        res = GT(df).fmt_integer(columns="col1", rows=every_n_row(-1), scale_by=100)
    assert "`n` must be a positive integer greater than 0." in exc_info.value.args[0]

    with pytest.raises(ValueError) as exc_info:
        res = GT(df).fmt_integer(columns="col1", rows=every_n_row(2, -1), scale_by=100)
    assert "`offset` must not be a negative integer." in exc_info.value.args[0]

    with pytest.raises(ValueError) as exc_info:
        res = GT(df).fmt_integer(columns="col1", rows=every_n_row(2, 2), scale_by=100)
    assert "`offset` must be less than `n_rows`." in exc_info.value.args[0]

    with pytest.raises(ValueError) as exc_info:
        res = GT(df).fmt_integer(columns="col1", rows=every_n_row(2, 3), scale_by=100)
    assert "`offset` must be less than `n_rows`." in exc_info.value.args[0]

    with pytest.raises(ValueError) as exc_info:
        res = GT(df).fmt_integer(columns="col1", rows=every_n_row(100), scale_by=100)
